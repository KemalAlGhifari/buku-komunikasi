                ...
                <div id="ctable">
                  <table  id="table" class="table table-striped">
                    ...
                    <tr style="text-align: center;border-bottom: 1px solid rgb(170, 170, 170)">
                        <td style="width: 2%;">{{$no++}}</td>
                        <td style="width: 15%;">{{$itemdash2->nama}}</td>
                        <td style="width: 15%;">@foreach ($itemdash2->RelationTokelas as $items)
                            {{$items->nama_kelas}}
                        </td>
                        <td style="width: 20%;">@foreach ($itemdash->RelasiToPelanggaranFromDash as $itemdashs)
                            {{$itemdashs->pelanggaran}}
                        @endforeach</td>
                          <td>{{$itemdash->tanggal}}</td>
                          <td>@foreach ($itemdash->RelasiToPelanggaranFromDash as $itemdashs)
                            {{$itemdashs->point}}
                        </td>
                        <td>
                          <div style="display: flex;justify-content: center;gap: 10px;">
                            <button style="height: 25px;display: flex;align-items: center;width: 60px;justify-content: center;font-size: 14px"  type="button" class="btn btn-primary" data-id="{{$itemdash->id}}" data-nama='{{$itemdash2->nama}}' data-nisn='{{$itemdash2->nisn}}' data-kelas='{{$items->nama_kelas}}' data-jurusan='{{$itemdash2->jurusan}}' data-jenis='{{$itemdash2->jenis_kelamin}}' data-pelanggaran='{{$itemdashs->pelanggaran}}' data-point='{{$itemdashs->point}}' data-sanksi='{{$itemdashs->sanksi}}'  data-no='{{$itemdash2->no_telepon}}' data-tanggal="{{$itemdash->tanggal}}" data-email='{{$itemdash2->email}}'data-bs-toggle="modal"   data-bs-target="#view">
                              <span style="font-size: 13px">View</span>
                            </button>

                          
                            <button style="height: 25px;display: flex;align-items: center;width: 60px;justify-content: center;font-size: 14px"  type="button" class="btn btn-danger" data-id='{{$itemdash->id}}' data-pelanggaran='{{$itemdashs->pelanggaran}}' data-point='{{$itemdashs->point}}' data-sanksi='{{$itemdashs->sanksi}}'  data-nama='{{$itemdash2->nama}}' data-tanggal="{{$itemdash->tanggal}}" data-email='{{$itemdash2->email}}'data-bs-toggle="modal"   data-bs-target="#hapus">
